...
class AllTheBoss implements IPostDBLoadMod, IPreSptLoadMod
{
  ...
  public preSptLoad(container: DependencyContainer): void
  {
    ...
    const preSptModLoader = this.container.resolve<PreSptModLoader>("PreSptModLoader")
  ...
  private addRandomHorde(minimumSupports: number, maximumSupports: number, map: string, locations: any): void
  {
    ...
    if (bigBossindex >= options.length) bigBossindex = options.length - 1;
    ...
    while (done === false && options.length > 0) 
    {
      ...
      if (supportIndex >= options.length) supportIndex = options.length - 1;
